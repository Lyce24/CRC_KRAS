"""
Core evaluation engine for Stage 4.
...
def compute_calibration_intercept_slope(
    ...
    """
    Logistic recalibration parameters on the logit scale (binary only).

    slope: fit  y ~ a + b * logit(p)  by Newton-Raphson; report b.
        b < 1 → predictions too extreme (overfitting); b > 1 → too moderate.
    intercept (calibration-in-the-large): fit  y ~ a + offset(logit(p))
        with slope fixed at 1; report a.
        a < 0 → systematic over-prediction; a > 0 → under-prediction.
    ...
    # ── Intercept-only model with offset (slope fixed at 1) ──────────────
    a = 0.0
    for _ in range(max_iter):
        mu = _sigmoid(lp + a)
        grad = float(np.sum(labels - mu))
    ...
    # ── Two-parameter model: y ~ a2 + b * logit(p) ───────────────────────
    # Damped Newton with step-halving on the NLL. With saturated/quasi-
    # separated probabilities the slope MLE does not exist (|b| → ∞); detect
    # that and report NaN rather than a runaway number.
    def _nll(t: np.ndarray) -> float:
        z = np.clip(X @ t, -500, 500)
        return float(np.sum(np.log1p(np.exp(z)) - labels * z))

    theta = np.array([0.0, 1.0])  # [intercept, slope]
    ...
    for _ in range(max_iter):
        ...
        try:
            ...
            break
        # Step-halving: accept only steps that do not increase the NLL.
        current = _nll(theta)
        scale = 1.0
        for _ in range(30):
            candidate = theta + scale * step
            if _nll(candidate) <= current + 1e-12:
                break
            scale *= 0.5
        else:
            break
    ...
    slope = float(theta[1])
    slope_unstable = (not np.isfinite(slope)) or abs(slope) > 50.0
    return {
        "calibration_intercept": float(a),
        "calibration_slope": float("nan") if slope_unstable else slope,
        "slope_model_intercept": float(theta[0]),
        "slope_converged": bool(converged and not slope_unstable),
    }
