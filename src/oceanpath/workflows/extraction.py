...
from oceanpath.extraction import (
...
def _run_metadata_output_dir(
    extraction_dir: Path,
    shard_id: int | None,
    total_shards: int | None,
) -> Path:
    """Give every array worker an isolated ``_run`` metadata directory."""

    if (shard_id is None) != (total_shards is None):
        raise ValueError("Slurm shard ID and count must be provided together")
    if shard_id is None or total_shards is None:
        return extraction_dir
    return extraction_dir / ".shard_runs" / f"shard_{shard_id:04d}_of_{total_shards:04d}"
...
def build_extraction_config(cfg: Any) -> TridentExtractionConfig:
    ...
    return TridentExtractionConfig(
        ...
        custom_mpp_keys=[str(value) for value in cfg_list(cfg, "extraction.custom_mpp_keys")]
        or None,
        ...
        target_mpp=(
            float(value)
            if (value := cfg_select(cfg, "extraction.target_mpp", None)) is not None
            else None
        ),
...
def run_extraction(cfg: Any) -> StageResult:
    ...
    raw_shard_id = cfg_select(cfg, "platform.slurm_array_task_id", None)
    raw_total_shards = cfg_select(cfg, "platform.slurm_array_task_count", None)
    shard_id = int(raw_shard_id) if raw_shard_id is not None else None
    total_shards = int(raw_total_shards) if raw_total_shards is not None else None
    context_output_dir = _run_metadata_output_dir(
    ...
    with run_context(
        ...
        if bool(cfg_select(cfg, "validate_only", False)):
            validate_inputs(extraction_cfg, tasks, create_job_dir=False)
            validation = validate_outputs(extraction_cfg, tasks)
            if validation["missing"] or validation["invalid"] or validation["unexpected"]:
                raise RuntimeError(
                    "Extraction validation failed: "
                    f"{len(validation['missing'])} slides missing and "
                    f"{len(validation['invalid'])} invalid; "
                    f"{len(validation['unexpected'])} unexpected"
                )
            details = validation
            status = "validated"
        else:
