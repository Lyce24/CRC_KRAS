"""
Stage 4: Basic model evaluation.

Reads Stage 3 training outputs and produces:
...
def evaluate_final_models(
    ...
    for strategy in strategies:
        ...
        if skip_inference:
            ...
            try:
                configured_workers = OmegaConf.select(
                    cfg, "training.num_workers", default=None
                )
                num_workers = (
                ...
                configured_prefetch = OmegaConf.select(
                    cfg, "training.prefetch_factor", default=None
                )
                prefetch_factor = (
                ...
                configured_eval_batch = OmegaConf.select(
                    cfg, "training.eval_batch_size", default=None
                )
                eval_batch_size = (
                    int(cfg.training.batch_size)
                    if configured_eval_batch in (None, "null")
                    else int(configured_eval_batch)
                )
                preds_df = run_inference(
                    ...
                    force_float32=OmegaConf.select(
                        cfg, "training.force_float32", default=True
                    ),
                    eval_fixed_bag_size=OmegaConf.select(
                        cfg, "training.eval_fixed_bag_size", default=None
                    ),
                    short_bag_policy=OmegaConf.select(
                        cfg, "training.short_bag_policy", default="repeat"
                    ),
                    cap_strategy=OmegaConf.select(
                        cfg, "training.cap_strategy", default="random"
                    ),
                    cap_grid_size=OmegaConf.select(
                        cfg, "training.cap_grid_size", default=32
                    ),
