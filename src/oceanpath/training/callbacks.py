...
MinEpochEarlyStopping:  EarlyStopping with a minimum number of epochs before
                        a stop may be issued (checkpointing is unaffected).
"""
...
class MinEpochEarlyStopping(EarlyStopping):
    """EarlyStopping that cannot fire before ``min_epoch_before_stop``.

    Gates STOPPING only, never checkpointing. ModelCheckpoint is a separate
    callback and keeps tracking the monitored metric from epoch 0, so the
    restored weights are always the best epoch seen — even if that epoch lies
    inside the protected window. Concretely: with min_epoch_before_stop=10 and
    the best validation loss at epoch 3 that never improves, training runs to
    epoch 10 and then restores the epoch-3 weights.

    The patience counter also runs from epoch 0, so the protected window is a
    floor on training length, not a reset. Once the floor is passed, a run that
    has already exhausted its patience stops at the next check rather than
    serving a further ``patience`` epochs.

    Why have a floor at all: with a short schedule and a noisy patient-level
    criterion, an unlucky first few epochs can trip patience before the cosine
    schedule has done anything, and the fold then reports a barely-trained
    model as if it had converged.
    """

    def __init__(self, *args, min_epoch_before_stop: int = 0, **kwargs):
        super().__init__(*args, **kwargs)
        self.min_epoch_before_stop = int(min_epoch_before_stop)

    def _run_early_stopping_check(self, trainer) -> None:
        # current_epoch is 0-based; "min_epoch_before_stop=10" means at least
        # 10 completed epochs before a stop may be issued.
        if trainer.current_epoch + 1 < self.min_epoch_before_stop:
            # Still update the best score / patience counter, just never stop.
            should_stop, reason = self._evaluate_stopping_criteria(
                self._extract_monitor_value(trainer)
            )
            if should_stop and self.verbose:
                logger.info(
                    "Early stopping criterion met at epoch %d but held: "
                    "min_epoch_before_stop=%d. Checkpoint selection is unaffected.",
                    trainer.current_epoch + 1,
                    self.min_epoch_before_stop,
                )
            return
        super()._run_early_stopping_check(trainer)

    def _extract_monitor_value(self, trainer) -> torch.Tensor:
        """The monitored metric for this epoch, as EarlyStopping would read it."""
        logs = trainer.callback_metrics
        current = logs.get(self.monitor)
        if current is None:
            return torch.tensor(float("nan"))
        return current if isinstance(current, torch.Tensor) else torch.tensor(current)
