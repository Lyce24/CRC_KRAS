...
def test_foundation_dag_launchers_exist():
    """The DAG executes these files in child processes; imports are not enough."""
    scripts_dir = Path(__file__).resolve().parents[2] / "scripts"
    missing = [name for name in FOUNDATION_LAUNCHERS if not (scripts_dir / name).is_file()]
    assert not missing, f"foundation DAG launchers are missing: {missing}"
...
def test_essential_configs_compose_with_reference_data(config_name):
    config_dir = str(Path(__file__).resolve().parents[2] / "configs")
    with initialize_config_dir(config_dir=config_dir, version_base="1.3"):
        cfg = compose(config_name=config_name)

    OmegaConf.to_container(cfg, resolve=True)
    assert cfg.data.name.startswith("aim1")
...
def test_local_platform_uses_portable_environment_root(monkeypatch):
    ...
    assert cfg.data.slide_dir == f"{root}/slides/colon"
...
def test_colon_extraction_profile_pins_one_spatial_contract():
    config_dir = str(Path(__file__).resolve().parents[2] / "configs")
    with initialize_config_dir(config_dir=config_dir, version_base="1.3"):
        cfg = compose(
            config_name="extract",
            overrides=["data=colon", "encoder=conch", "extraction=colon"],
        )

    assert cfg.encoder.patch_size == 512
    assert cfg.extraction.patch_size == 256
    assert cfg.extraction.mag == 20
    assert cfg.extraction.target_mpp == 0.5
    assert cfg.extraction.overlap == 0
    assert cfg.extraction.reader_type == "openslide"
    assert cfg.extraction.search_nested is False
    assert cfg.extraction.coords_subdir == "20x_256px_0px_overlap_mpp0.5"
    assert cfg.data.filename_column == "Slide_ID"
    assert cfg.data.patient_id_column == "Patient_ID"
    assert list(cfg.data.label_columns) == []
...
def test_extraction_dry_run_does_not_create_output(tmp_path):
    ...
    job_dir = tmp_path / "features" / "colon"
...
@pytest.mark.parametrize(
    "config_name",
    ["train", "evaluate", "export", "pipeline"],
)
...
def test_task_metadata_drives_evaluation_and_export():
    config_dir = str(Path(__file__).resolve().parents[2] / "configs")
    with initialize_config_dir(config_dir=config_dir, version_base="1.3"):
...
def test_foundation_model_profiles_are_intentionally_small():
    model_dir = Path(__file__).resolve().parents[2] / "configs" / "model"
    ...
    cfg = OmegaConf.create(
        {
            ...
            "encoder": {"name": "uni"},
            "splits": {"name": "kfold5"},
            "train_dir": "/data/outputs/train/colon_reference",
            "eval_dir": "/data/outputs/train/colon_reference/eval",
            "export": {"artifact_dir": "/data/artifacts/colon_reference"},
        }
    )

    paths = FoundationPaths.from_config(cfg)

    assert paths.dataset_name == "aim1_1a"
    assert paths.splits_dir == Path("/data/splits/aim1_1a/kfold5")
    assert paths.train_dir == Path("/data/outputs/train/colon_reference")
    assert paths.eval_dir == paths.train_dir / "eval"
    assert paths.artifact_dir == Path("/data/artifacts/colon_reference")
...
def test_domain_packages_operate_on_typed_values():
    ...
    """
    package_root = Path(__file__).resolve().parents[2] / "src" / "oceanpath"
    domain_packages = [
        "datasets",
        "eval",
        "export",
...
def test_project_specific_source_package_stays_dissolved():
    """Project branches extend the generic package directly, not under projects/."""
    package_root = Path(__file__).resolve().parents[2] / "src" / "oceanpath"
    assert not (package_root / "projects").exists(), (
        "Do not add oceanpath.projects; put reusable behavior in the existing "
        "domain package and keep cohort-specific entry points outside src/."
    )
...
def test_legacy_data_namespace_stays_dissolved():
    ...
    assert not legacy_modules, (
        "Use the explicit datasets, extraction, and splitting packages; "
        f"do not restore the ambiguous oceanpath.data namespace: {legacy_modules}"
    )


def test_stage_numbering_matches_the_five_stage_dag():
    root = Path(__file__).resolve().parents[2]
    search_roots = [root / "src", root / "configs", root / "scripts", root / "tools"]
    candidates = [path for search_root in search_roots for path in search_root.rglob("*")]
    candidates += root.glob("phase*.py")
    stale = []
    for path in candidates:
        if path.suffix not in {".py", ".yaml", ".md"}:
            continue
        source = path.read_text()
        if any(f"Stage {number}" in source for number in (6, 7, 8, 9)):
            stale.append(str(path.relative_to(root)))
    assert not stale, f"Stale stage numbering outside the five-stage DAG: {stale}"
