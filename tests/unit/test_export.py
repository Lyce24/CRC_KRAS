...
def _reference_config() -> dict:
    return {
        ...
        "num_classes": 2,
        "data": {"name": "aim1_1a", "label_columns": ["Binary WHO 2022"]},
...
def test_model_card_uses_canonical_fingerprint():
    ...
    assert card["data"]["dataset"] == "aim1_1a"
...
def test_tiny_checkpoint_exports_real_onnx_and_torchscript(tmp_path):
    ...
    # The serving backend was removed with the deployment surface; export is
    # still exercised end to end by loading the ONNX artifact directly.
    import onnxruntime

    session = onnxruntime.InferenceSession(str(tmp_path / "artifact" / "model.onnx"))
    inputs = {session.get_inputs()[0].name: torch.randn(1, 3, 8).numpy()}
