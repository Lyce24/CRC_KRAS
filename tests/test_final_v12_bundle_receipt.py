...
from tools import final_v12_bundle_receipt as verifier  # noqa: E402
...
def _side_document(title: str) -> str:
    return f"""# FINAL-v12 {title}

The sealed FINAL-v11 receipt is present and authoritative.
...
def _valid_results() -> str:
    return """# FINAL-v12 integrated results for paper selection
...
def _fixture(tmp_path: Path, *, source_count: int = 4) -> verifier.BundlePaths:
    ...
    final_dir = repo / "reports/final_v12"
    ...
    parent_documents = {
        "Experimental_Setup.md": "# FINAL-v11 setup\n\nSealed.\n",
        "Results.md": "# FINAL-v11 results\n\nSealed.\n",
        "Audit.md": "# FINAL-v11 audit\n\nSealed.\n",
    }
    ...
    (final_dir / "Audit.md").write_text(_audit(sources), encoding="utf-8")
...
def test_production_parent_pins_match_sealed_final_v11() -> None:
    ...
    assert verifier.EXPECTED_SOURCE_COUNT == 139
...
def test_final_manifest_strict_json_rejects_nonfinite_constants(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
    manifest = paths.final_v12 / verifier.SOURCE_MANIFEST_NAME
...
def test_parent_document_drift_fails_receipt_identity_replay(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        verifier.BundleVerificationError, match="FINAL-v11 Results.md identity drift"
    ):
        verifier.build_receipt(paths)


def test_final_manifest_may_differ_only_in_bundle_and_status(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
    manifest_path = paths.final_v12 / verifier.SOURCE_MANIFEST_NAME
    manifest = json.loads(manifest_path.read_text())
    manifest["artifacts"][1]["role"] = "mutated_role"
    _write_json(manifest_path, manifest)
    with pytest.raises(verifier.BundleVerificationError, match="beyond bundle/status metadata"):
        verifier.build_receipt(paths)
...
def test_results_topology_fails_closed(
    ...
    paths = _fixture(tmp_path)
    results = paths.final_v12 / "Results.md"
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "No FINAL-v11 receipt has been published.",
...
def test_audit_source_index_must_be_exact(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
    audit = paths.final_v12 / "Audit.md"
...
def test_seal_is_atomic_exactly_once_and_verifiable(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
...
def test_existing_receipt_symlink_is_never_overwritten(tmp_path: Path) -> None:
    paths = _fixture(tmp_path)
    decoy = paths.final_v12 / "decoy.json"
