.PHONY: sync config lint precommit typecheck lock-check test test-unit test-extraction test-cov build ci
...
config:
	uv run pytest -q tests/unit/test_foundation.py
...
test-unit:
	uv run pytest tests/unit -m "not slow and not gpu and not integration"

test-extraction:
	uv run pytest tests/extraction -m "not slow and not gpu and not integration"
